...
async function main() {
  ...
  log("info", "boot", { project: "gate-io-alt-trading-bot", venue: "gate", mode, kind: "listing" });

  if (mode === "live") {
    assertLiveAllowed(settings, args.confirmLive, "GATE");
    const broker = createCcxtBroker(settings, "GATE", "gate");
