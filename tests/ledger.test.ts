...
test("gate-io-alt-trading-bot ledger summary", () => {
  const l = new Ledger();
...
test("gate-io-alt-trading-bot consecutive losses", () => {
  assert.equal(maxConsecutiveLosses([-1, -2, 1, -1]), 2);
});

test("gate-io-alt-trading-bot retry eventually succeeds", async () => {
  let n = 0;
...
test("gate-io-alt-trading-bot price series sma", () => {
