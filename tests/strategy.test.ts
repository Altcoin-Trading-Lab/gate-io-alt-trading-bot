...
test("gate-io-alt-trading-bot settings load", () => {
  const s = loadSettings(process.cwd());
  assert.equal(s.venue, "gate");
  assert.ok(s.risk.maxDailyLossUsd > 0);
});

test("gate-io-alt-trading-bot paper strategy steps", async () => {
  const s = loadSettings(process.cwd());
...
test("gate-io-alt-trading-bot risk guardian trips daily loss", () => {
  const s = loadSettings(process.cwd());
