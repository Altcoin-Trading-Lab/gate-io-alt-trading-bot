...
const ID="gate-io-alt-trading-bot";
const rand=mulberry32(hashId(ID));
const venues=['Binance','OKX','Bybit','Bitget','Gate'];
const assets=['BTC','ETH','SOL','ARB','DOGE','OP'];
const grid=assets.map(()=>venues.map(()=>(rand()-0.4)*0.9));
drawHeat(document.getElementById('heat'),grid,[cssVar('--a'),cssVar('--c'),cssVar('--warn')]);
const eq=[]; let e=1000; for(let i=0;i<30;i++){e+=10+rand()*28-(rand()>0.85?40:0); eq.push(e)}
drawEquity(document.getElementById('eq'),eq,cssVar('--a'),1000);
